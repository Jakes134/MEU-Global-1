...
try {
const { rows } = await pool.query(`INSERT INTO posts (client_id, title, caption, platforms, post_date, post_time, status, approval_status, created_by) VALUES ($1,$2,$3,$4,$5,$6,$7,'pending',$8) RETURNING *`, [client_id || null, title, caption, platforms, post_date, post_time || null, status || ‘draft’, created_by]);
res.json(rows[0]);
} catch (err) {
res.status(500).json({ error: err.message });
}
...
try {
const { rows } = await pool.query(`UPDATE posts SET title=$1, caption=$2, platforms=$3, post_date=$4, post_time=$5, status=$6, approval_status='pending', is_approved=FALSE, rejection_reason=NULL WHERE id=$7 RETURNING *`, [title, caption, platforms, post_date, post_time || null, status, req.params.id]);
res.json(rows[0]);
} catch (err) {
res.status(500).json({ error: err.message });
}
...
try {
const { rows } = await pool.query(‘INSERT INTO task_comments (task_id, user_id, comment) VALUES ($1,$2,$3) RETURNING *’, [req.params.id, user_id, comment]);
res.json(rows[0]);
} catch (err) {
res.status(500).json({ error: err.message });
}
