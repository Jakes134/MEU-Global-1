...
`SELECT p.name, count(ec.id) as count FROM products p JOIN end_customers ec ON ec.product_id=p.id ${client_id && client_id !== 'null' ? ' WHERE p.client_id=$1' : ''} GROUP BY p.name ORDER BY count DESC LIMIT 5`, p
);
...
// Update post date only (for drag and drop)
...
try {
const { rows } = await pool.query(
`SELECT tc.*, u.name as author_name FROM task_comments tc LEFT JOIN users u ON tc.user_id=u.id WHERE tc.task_id=$1 ORDER BY tc.created_at ASC`,
[req.params.id]
);
res.json(rows);
} catch (err) { res.status(500).json({ error: err.message }); }
