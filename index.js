...
app.post('/api/admin/add-user', async function(req, res) {
  ...
  try {
    var rawPassword = (password && password.trim()) ? password : 'ChangeMe123!';
    var hashed = await bcrypt.hash(rawPassword, 12);
    var result = await pool.query('INSERT INTO users (name, email, password, role, client_id, must_change_password) VALUES ($1, $2, $3, $4, $5, TRUE) RETURNING id, name, email, role', [name.trim(), email.toLowerCase().trim(), hashed, role || 'user', client_id || null]);
...
app.get('/api/users/assignable', async function(req, res) {
  try {
    var result = await pool.query("SELECT id, name, email, role FROM users WHERE role IN ('user', 'admin') ORDER BY name ASC");
    res.json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
