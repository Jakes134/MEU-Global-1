...
// Google Generative AI - Optional (app works without it)
let GoogleGenerativeAI = null;
try {
  GoogleGenerativeAI = require('@google/generative-ai').GoogleGenerativeAI;
  console.log('✅ Google Generative AI loaded - AI features enabled');
} catch (e) {
  console.log('⚠️  Google Generative AI not installed - AI features disabled');
  console.log('   To enable: npm install @google/generative-ai');
}
...
app.post('/api/chat', async (req, res) => {
  ...
  // Check if AI package is loaded
  if (!GoogleGenerativeAI) {
    return res.status(503).json({
      error: 'AI features are not available. Please install @google/generative-ai package and restart the server.',
      details: 'Run: npm install @google/generative-ai'
    });
  }
...
// Approve/Reject Landing Page
app.put('/api/pages/:id/approve', async (req, res) => {
  const { id } = req.params;
  ...
  try {
    if (approved) {
      await pool.query(
        `UPDATE landing_pages SET approval_status = 'approved', approved_by = $1, approved_at = CURRENT_TIMESTAMP WHERE id = $2`,
        [reviewer_id, id]
      );
    } else {
      await pool.query(
        `UPDATE landing_pages SET approval_status = 'rejected', rejection_reason = $1, approved_by = $2, approved_at = CURRENT_TIMESTAMP WHERE id = $3`,
        [rejection_reason, reviewer_id, id]
      );
    }
    res.json({ success: true });
  } catch (err) {
...
app.put('/api/campaigns/:id/approve', async (req, res) => {
  ...
  try {
    if (approved) {
      await pool.query(
        `UPDATE email_campaigns SET approval_status = 'approved', approved_by = $1, approved_at = CURRENT_TIMESTAMP WHERE id = $2`,
        [reviewer_id, id]
      );
    } else {
      await pool.query(
        `UPDATE email_campaigns SET approval_status = 'rejected', rejection_reason = $1, approved_by = $2, approved_at = CURRENT_TIMESTAMP WHERE id = $3`,
        [rejection_reason, reviewer_id, id]
      );
    }
    res.json({ success: true });
...
// Get pending pages for approval (client owners)
app.get('/api/pages/pending-approval', async (req, res) => {
  const { client_id } = req.query;
  try {
    let query = `SELECT lp.*, c.name as client_name, u.name as creator_name FROM landing_pages lp LEFT JOIN clients c ON lp.client_id = c.id LEFT JOIN users u ON lp.created_by = u.id WHERE lp.approval_status = 'pending'`;
    const params = [];

    if (client_id) {
      query += ` AND lp.client_id = $1`;
      params.push(client_id);
    }
    ...
    console.error('Error fetching pending pages:', err);
    res.status(500).json({ error: 'Database error' });
  }
});

// Get pending campaigns for approval (client owners)
app.get('/api/campaigns/pending-approval', async (req, res) => {
  const { client_id } = req.query;
  try {
    let query = `SELECT ec.*, c.name as client_name, u.name as creator_name FROM email_campaigns ec LEFT JOIN clients c ON ec.client_id = c.id LEFT JOIN users u ON ec.created_by = u.id WHERE ec.approval_status = 'pending'`;
    const params = [];

    if (client_id) {
      query += ` AND ec.client_id = $1`;
      params.push(client_id);
    }
